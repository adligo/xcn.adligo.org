...
XCN is a binary exchange notation system which relies heavily on UTF-8 text based data.  It is largely a derivative of EJCN, XML, JSON, and their predecessors, including HTML and the Apache HTTP Server configuration files.  Unlike JSON and XML, which are context-free grammars XCN is a hybrid context-sensitive and context-free grammar.  In addition, XCN's goals were to improve upon JSON (the modern de-facto data interchange standard) by adding the following improvements;
